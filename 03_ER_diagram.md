...
- 一つの注文で購入ステータスは同じになるはずなので、商品ごとにステータスを設ける必要はありません。決済テーブルを受注テーブルとし、一度の注文に関する情報として集約させることができます。
- 購入した商品内容をどこで確認出来るのでしょうかが分かりません。テーブル構成を再度考えてみてください。
...
- ステータス(取り扱い)は、具体的に何のステータスを示しているか一見しただけで分かるでしょうか。どのステータスなのかがわかるような名前にしましょう。
- 商品に複数枚のディスクがある場合は、どのように保存するのでしょうか。
- アーティスト名、レーベル名、ジャンル名は直接入力させるのでしょうか。1人のアーティストが複数の曲を販売する可能性があります。その都度同じ名前のアーティストを入力させるのは全角文字でデータが大きくなります。またアーティスト名を変更したくなった場合は大量のレコードをUPDATEする必要があるので、商品テーブルに入力させるのは好ましくありません。レーベル名,ジャンル名も同様です。
...
- 商品テーブルと関連付けがされているので、アーティスト名、ジャケット画像、シングル/アルバム名のカラムは不要です。
...
- 購入日カラムがありますが、ActiveRecordでデフォルトで作られる登録日で代替可能です。
- 住所カラムが存在しますが、発送先の情報としてこれだけで十分でしょうか。宛先名や郵便番号
- カートテーブルから決済テーブルにリレーションが書かれていますが、決済のFKにカートidが記述されていません。また決済テーブルとカートテーブルのリレーションは本当に必要でしょうか。
-カートテーブルと決算テーブル間にリレーションがありますが、FKが見当たりません。
 適切にFKが設定されていないので、これでは購入時の合計金額を算出する事が出来ません。

## 購入履歴
- 購入後に値下げが発生した場合、購入時の価格は確認できますか。
- 何を購入したか判別できますか。
- ステータス(発送状況)は、購入した商品ごとではなく、注文の属性として持つべきものです。
...
* 運営上「いつ,どの商品が入荷したか」という情報を管理しておく方が安全な運用です。
 - 入荷数の証拠にもなります。
- ディスク枚数が存在しますが、ディスクテーブルのデータから算出できるため不要です

## ディスクテーブル
- 1枚目の1曲目と2枚目の1曲目の区別はつけられますか？
- ディスク1枚に対して複数商品が存在するような書き方となっておりますが、想定通りでしょうか。
- トラックNoとありますが、これはディスク内での曲順を表す番号ですので、持たせるテーブルが異なります。

## 曲
- ディスクと曲のリレーションが逆になっています。ディスクと商品のリレーションの時と同様のレビュー文を追記してください。
- 一曲に対して複数ディスクが存在するような書き方となっておりますが、想定通りでしょうか。
- ディスクと曲のリレーションが逆になっています。ディスクと商品のリレーションの時と同様のレビュー文を追記してください。
- 曲順はどのように保存されるのでしょうか。
...
- 購入日カラムが記述されていたのですが、created_atと同日になると思います。別で持った理由はありますか。(間違いではないので、理由があれば知りたいです)
- 配送状況はどのように管理するのでしょうか。

## 購入詳細
- 何を購入したか判別できますか。
- 一つの購入商品に対して複数の購入履歴が存在するような書き方となっておりますが、想定通りでしょうか。
- データベースの情報は管理者が利用しますので、テーブル名は「購入」同様、「受注詳細」「注文詳細」のような命名にする必要があります。管理者側から見た呼び方にしておきましょう。
